...
class ReceiptController(BaseController):
	...
	def setVoucher(self):
		...
		if c.flag == "n":
			...
			c.flag = "n"
			receipt = app_globals.server_proxy.account.getAllAccountNamesByLedger(session["gnukhata"])
			receiptacc = []
			#populate existing account list
			for db in receipt:
				receiptacc.append(db[0])
			c.bank = receiptacc
			res = app_globals.server_proxy.voucher.getVoucherCode(session["gnukhata"])
			if res == False:
				c.vouchercode = 1
		...
		if c.flag == "e":
			...
			response.headerlist = [['Content-type','text-html']]
			response.charset='utf8' 
		return render('/receipt voucher.mako')
	...
	def getVoucher(self):
		...
		if str(request.params["submit"]) == 'Edit':
			...
			c.result=res	
			c.flag = 'e'
			receipt = app_globals.server_proxy.account.getAllAccounts(session["gnukhata"])
			receiptacc = []
			#populate existing account list
			for db in receipt:
				receiptacc.append(db[0])
		...
		if str(request.params["submit"]) == 'Open':
			...
			response.headers["Content-disposition"] = "attachment; filename=receiptreport.pdf"
			...
			ps.append(Paragraph(" <para align=left>Payment Voucher</para>",styleT))
