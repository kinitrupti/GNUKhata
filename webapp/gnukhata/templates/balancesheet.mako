...
$(document).ready(function()
{
	...
	$("#viewtb").click(function()
	{
		...
		var label = document.getElementById('bs_type').value;
		
		if (label == "--Please Select One--")
		{
		alert("Please select the Balance Sheet Type");
		return false;
		}
		else
		{
		...
		if (start.getTime() <= new_transaction_date.getTime() && new_transaction_date .getTime()<= end.getTime())
		{
			var bs_type = $('select#bs_type').val();
			$("#balsheet").load(location.protocol+"//"+location.host+"/reports/createBalanceSheet",{'to_date': todate,'bs_type': bs_type});
			return false;
		}
...
<script>
/*Function to go from bstype to datey by Up arrow*/
function bstypetodatey()
{
	var x=document.getElementById("bs_type").selectedIndex;
	var y=document.getElementById("bs_type").options;
	if(y[x].index==0){
		if (code==38){
			document.getElementById('datey').focus();
		}
...
<div id="tab1" class="tab_content">
	<form id="balsheet" method = "post" action="${h.url_for(controller='reports',action='createBalanceSheet')}">
...
<fieldset id="fieldset" style="width:60%;">				
	<legend><label for ="view trialbal"><b> View Balance Sheet</b></label></legend>
	...
	<table width="90%" border="0" id="tbinput_table">
		...
		<tr align="center">
			...
			<input type="text" size="1" maxlength="2" id="dated" name="dated" class="dated" onkeyup="keydown(event,'datem')" onKeydown="return disableEnterKey(event)">-
						<input type="text" maxlength="2" id="datem" size="1" name="datem" class="datem" onKeydown="return disableEnterKey(event)" onkeyup="keydown(event,'datey')" onkeypress="keyup(event,'dated')" >-
						<input type="text" size="4" maxlength="4" id="datey" name="datey" class="datey" onKeydown="return disableEnterKey(event)" onkeypress="keyup(event,'datem')" onkeyup="keydown(event,'bs_type')"></td><br>
			</tr>
			<tr align="center">
			<td ><br><label for="bs_type" id="label_bs_type"><font size = "3"><b>&nbsp;&nbsp;Balance Sheet Type:</b> </label>
			<select id="bs_type" name="bs_type" onkeyup="keydown(event,'viewtb')" onkeypress="bstypetodatey()" >
				<option>--Please Select One--</option>
				<option>Conventional Balance Sheet</option>
				<option>Sources & Application of Funds</option>
			</select><br><br><br>
		</td>
		</tr>
		<tr align="right">
			<td><br><br><br>
			<input type="submit" name="submit" id="viewtb" onkeyup="keyup(event,'bs_type')"  value="View" src="/images/button.png"></td>
